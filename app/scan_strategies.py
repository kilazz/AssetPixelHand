...
# FIX: Correctly and separately handle optional imports for imagehash and scipy.
try:
    import imagehash
...
class FindDuplicatesStrategy(ScanStrategy):
        ...
        for paths in self.hash_map.values():
            if len(paths) > 1:
                ...
                if group_fps:
                    ...
                    exact_groups[best_fp] = {(fp, 100, "xxHash") for fp in group_fps if fp != best_fp}
    ...
    def _find_perceptual_duplicates(
        ...
        for group_paths in groups:
            if len(group_paths) > 1:
                ...
                if group_fps:
                    best_fp = find_best_in_group(group_fps)
                    phash_groups[best_fp] = {(fp, 100, "pHash") for fp in group_fps if fp != best_fp}
    ...
    def _finalize_all_results(self, *results_dicts: DuplicateResults) -> DuplicateResults:
        """
        Merges all found groups transitively using a graph-based approach with SciPy
        to ensure all interconnected duplicates form a single group.
        """
        self.state.set_phase("Finalizing and merging all results...", 0.0)
        if not SCIPY_AVAILABLE:
            self.signals.log.emit("Scipy not found, skipping final merge. Groups may be split.", "warning")
            final_groups = {}
        ...
        for results in results_dicts:
            for best_fp, dups in results.items():
                all_fps[best_fp.path] = best_fp
                for dup_fp, score, method in dups:
                    all_fps[dup_fp.path] = dup_fp
                    key = tuple(sorted((best_fp.path.as_posix(), dup_fp.path.as_posix())))
                    dup_info[key] = (score, method)

        path_list = list(all_fps.keys())
        path_to_idx = {path: i for i, path in enumerate(path_list)}
        num_files = len(path_list)
        rows, cols, data = [], [], []

        for (path1, path2), _ in dup_info.items():
            idx1, idx2 = path_to_idx[Path(path1)], path_to_idx[Path(path2)]
            rows.extend([idx1, idx2])
            cols.extend([idx2, idx1])
            data.extend([1, 1])

        graph = sparse.csr_matrix((data, (rows, cols)), shape=(num_files, num_files))
        _, labels = connected_components(csgraph=graph, directed=False, return_labels=True)

        components = defaultdict(list)
        for i, label in enumerate(labels):
            components[label].append(all_fps[path_list[i]])

        final_groups: DuplicateResults = {}
        for component_fps in components.values():
            if len(component_fps) < 2:
                continue
            ...
            for fp in component_fps:
                ...
                score, method = dup_info.get(key, (100, "AI"))  # Default for transitive links

                dups_set.add((fp, score, method))
