...
class ScannerCore(QObject):
    ...
    def run(self, stop_event: threading.Event):
        ...
        try:
            if self.config.scan_mode == "duplicates":
                self.run_find_duplicates(stop_event, start_time)
            elif self.config.scan_mode in ["text_search", "sample_search"]:
                self.run_search(stop_event, start_time)
            else:
    ...
    def run_search(self, stop_event: threading.Event, start_time: float):
        """Executes the pipeline for text or image-based similarity search."""
        if not self._setup_lancedb():
            return

        all_files = self._find_files(stop_event)
        if self._check_stop_or_empty(stop_event, all_files, self.config.scan_mode, [], start_time):
            return

        success, skipped = self._generate_fingerprints(all_files, stop_event)
        ...
        num_found, payload = len(search_results), []
        if num_found > 0:
        ...
        self._finalize_scan(payload, num_found, self.config.scan_mode, duration, self.all_skipped_files)
    ...
    def run_find_duplicates(self, stop_event: threading.Event, start_time: float):
        """Executes the full pipeline for finding duplicate and similar images."""
        if not self._setup_lancedb():
            return

        all_files = self._find_files(stop_event)
        if self._check_stop_or_empty(stop_event, all_files, "duplicates", [], start_time):
            return
        exact_groups, files_for_ai = self._find_exact_duplicates(all_files, stop_event)
        if stop_event.is_set():
            return
        success, skipped = self._generate_fingerprints(files_for_ai, stop_event)
        self.all_skipped_files.extend(skipped)
        if not success:
            self._check_stop_or_empty(stop_event, [], "duplicates", exact_groups, start_time)
            return
