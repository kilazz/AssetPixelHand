...
class FindDuplicatesStrategy(ScanStrategy):
    def execute(self, stop_event: threading.Event, start_time: float):
        context = ScanContext(
            ...
            session_conn=self.scanner_core.session_conn,
            vectors_table_name=self.scanner_core.vectors_table_name,
        )
    ...
    def _build_pipeline(self) -> list[tuple[ScanStage, float]]:
        ...
        if self.config.use_ai:
            pipeline.extend(
                [(FingerprintGenerationStage(), 0.40), (DatabaseIndexStage(), 0.05), (AILinkingStage(), 0.15)]
            )
        else:
...
class SearchStrategy(ScanStrategy):
    def execute(self, stop_event: threading.Event, start_time: float):
        ...
        pipeline_manager = PipelineManager(
            ...
            state=self.state,
            signals=self.signals,
            db_connection=self.scanner_core.session_conn,
    ...
    def _perform_similarity_search(self) -> tuple[int | None, str | None]:
        ...
        vec_literal = f"[{', '.join(map(str, query_vector.tolist()))}]"
        table_name = self.scanner_core.vectors_table_name
        ...
        query = f"""
            SELECT path, channel, array_cosine_similarity(vector, {vec_literal}::FLOAT[{self.config.model_dim}]) as sim
            FROM {table_name}
        ...
        hits = self.scanner_core.session_conn.execute(query).fetchall()

        results = []
        for path, channel, sim in hits:
            path_key = Path(path)
            if path_key in self.scanner_core.all_image_fps:
                fp_orig = self.scanner_core.all_image_fps[path_key]
                fp_copy = copy.copy(fp_orig)
                fp_copy.channel = channel
                results.append((fp_copy, 1.0 - sim))
