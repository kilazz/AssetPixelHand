...
def _process_metadata_entry(entry: tuple) -> ImageFingerprint | None:
    """Worker function to process a single file entry for metadata."""
...
class ScanStrategy(ABC):
    ...
    def _find_files_as_list(self, stop_event: threading.Event) -> list[Path]:
        """Finds all image files and returns them as a single list. (Used by SearchStrategy)"""
        ...
        for batch in finder.stream_files(stop_event):
            if stop_event.is_set():
                break
            files.extend(path for path, _ in batch)
...
class FindDuplicatesStrategy(ScanStrategy):
    ...
    def __init__(self, *args):
        ...
        self.links = defaultdict(lambda: (101.0, "Unknown"))
    ...
    def _add_link(self, path1: Path, path2: Path, score: float, method: str):
        ...
        new_link_is_better = method_priority[method] > method_priority[current_method] or (
            method == "AI" and method_priority[method] == method_priority[current_method] and score < current_score
        )
    ...
    def execute(self, stop_event: threading.Event, start_time: float):
        ...
        with ThreadPoolExecutor(max_workers=self.config.perf.num_workers * 2) as executor:
            futures = {
                executor.submit(_process_metadata_entry, entry)
                for batch in finder.stream_files(stop_event)
                for entry in batch
            }
            for future in as_completed(futures):
                ...
                if processed_count % 100 == 0:
                    self.state.update_progress(processed_count, finder.found_count)

        if stop_event.is_set():
            return
        if not self.all_image_fps:
    ...
    def _run_hashing_stages(self, all_files: list[Path], stop_event: threading.Event) -> list[Path]:
        ...
        with ctx.Pool(processes=num_workers) as pool:
            for i, (h, path) in enumerate(pool.imap_unordered(worker_get_xxhash, all_files, chunksize=200), 1):
                if stop_event.is_set():
                    return []
                if i % 500 == 0:
        ...
        if self.config.find_simple_duplicates:
            ...
            with ctx.Pool(processes=num_workers) as pool:
                for i, (dhash, path) in enumerate(
                    pool.imap_unordered(worker_get_dhash, reps_after_xxhash, chunksize=150), 1
                ):
                    if stop_event.is_set():
                        return []
                    if i % 300 == 0:
                        self.state.update_progress(i, len(reps_after_xxhash))
                    if dhash:
            ...
            for paths in dhash_map.values():
                rep = paths[0]
                reps_after_dhash.append(rep)
                if len(paths) > 1:
                    for follower in paths[1:]:
                        self._add_link(rep, follower, 100.0, "dHash")

        if not (self.config.find_perceptual_duplicates and IMAGEHASH_AVAILABLE and SCIPY_AVAILABLE):
            return reps_after_dhash

        self.state.set_phase("Phase 4/6: Finding near-duplicates (pHash)...", 0.20)
        phashes = []
        with ctx.Pool(processes=num_workers) as pool:
            for i, (phash, path) in enumerate(
                pool.imap_unordered(worker_get_phash, reps_after_dhash, chunksize=100), 1
            ):
                if stop_event.is_set():
                    return []
                if i % 200 == 0:
                    self.state.update_progress(i, len(reps_after_dhash))
                if phash:
        ...
        graph = sparse.csr_matrix((np.ones_like(rows), (rows, cols)), shape=(len(paths_ph), len(paths_ph)))
        _, labels = connected_components(csgraph=graph, directed=False, return_labels=True)

        files_for_ai = []
        components = defaultdict(list)
        for i, label in enumerate(labels):
            components[label].append(paths_ph[i])

        for group_paths in components.values():
            best_fp_in_phash_group = find_best_in_group([self.all_image_fps[p] for p in group_paths])
            files_for_ai.append(best_fp_in_phash_group.path)
            if len(group_paths) > 1:
                for path in group_paths:
                    if path != best_fp_in_phash_group.path:
                        self._add_link(best_fp_in_phash_group.path, path, 100.0, "pHash")

        self.signals.log.emit(
            f"Hashing filtered images. Sending {len(files_for_ai)} unique items for AI processing.", "info"
        )
        app_logger.info(f"pHash stage complete. Selected {len(files_for_ai)} representatives for AI stage.")
        return files_for_ai
    ...
    def _build_final_groups_from_links(self, stop_event: threading.Event) -> DuplicateResults:
        ...
        for (path1_posix, path2_posix), _ in self.links.items():
            idx1, idx2 = path_to_idx.get(Path(path1_posix)), path_to_idx.get(Path(path2_posix))
        ...
        for component_fps in (c for c in components.values() if len(c) > 1):
            ...
            for fp in component_fps:
                if fp != best_fp:
                    ...
                    score, method = self.links.get(
                        key, self.links.get(tuple(sorted((fp.path.as_posix(), best_fp.path.as_posix()))), (0.0, "AI"))
                    )

                    final_score = 0.0
                    if method == "AI":
                        final_score = self._score_to_percentage(score)
                    elif method in ["xxHash", "dHash", "pHash"]:
                        final_score = 100.0

                    dups_set.add((fp, int(final_score), method))
    ...
    def execute(self, stop_event: threading.Event, start_time: float):
        ...
        if num_found > 0:
            ...
            if self.config.scan_mode == ScanMode.SAMPLE_SEARCH and self.config.sample_path:
                best_fp = self._create_dummy_fp(self.config.sample_path)
                if best_fp:
